...
- **Selection:** each request goes to the authenticated, available account with the most headroom left. On the direct OAuth backend (default), headroom comes straight from Anthropic's own `anthropic-ratelimit-*` response headers — real remaining/limit for requests and tokens, refreshed on every call. Accounts with no live snapshot yet (freshly added, or served via the `cli` backend) fall back to fewest requests in the current local window; round-robin on ties. Pass an OpenAI `user` field or Anthropic `metadata.user_id` to keep a conversation pinned to one account.
- **Proactive sidelining:** if Anthropic reports zero remaining requests or tokens for an account and its reset window hasn't passed yet, the proxy marks it unavailable immediately — before it ever sends a request that would 429 — and routes to another account instead.
...
- **Usage tracking:** requests, tokens, and cost are counted per account over a rolling window (`USAGE_WINDOW_MS`, default 5h) and persisted to `<pool>/usage.json`, alongside Anthropic's real remaining/limit snapshot when the direct backend has seen one. The `cli` backend has no HTTP access to Anthropic's headers, so it falls back to observed usage plus the plan's `rateLimitTier` label only.
