...
- **Proactive sidelining:** if Anthropic reports a unified window fully consumed (`utilization` ≥ 1 or a `rejected` status) and that window hasn't reset yet, the proxy marks the account unavailable immediately — before it ever sends a request that would 429 — and routes to another account instead.
